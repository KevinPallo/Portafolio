...
<head>
	...
	<header>
			...
			<div class="Title-nombre">
				<h1 class="Title-nombre-texto">EDO cuarto orden</h1>
			</div>
			<nav>
				<ul>
		...
		<section class="subtitulos">
			...
			<section class="formulario-section">
				...
				<br>
				<form action="cuartoOrden.php" method="post">
					<div class="mb-3">
					...
					<div class="mb-3">
						...
						<input type="text" class="form-control" required name="Ax" placeholder="Ej: 4" value="<?php if (isset($_POST['n'])) echo $_POST['n']; ?>">	
					</div>
					<div class="mb-3">
						<label for="formGroupExampleInput2" class="form-label">Punto a evaluar </label>
						<input type="text" class="form-control" required name="num" placeholder="Ej: 7" value="<?php if (isset($_POST['valorEval'])) echo $_POST['valorEval']; ?>">
					</div>
					<div class="mb-3">
						<label for="formGroupExampleInput2" class="form-label">Valores (x,y) iniciales</label>
						<input type="text" name="error" class="form-control"  required placeholder="Ej: (1,2)" value="<?php if (isset($_POST['valoresIniciales'])) echo $_POST['valoresIniciales']; else echo ""; ?>">
					</div>
			...
			<?php
				if(isset($_POST['aceptar'])){
					...
					$y=$string[1];
					$h=($numEval-$x)/$n;
					CuartoOrden($x,$y,$h,$n,$funcion);
				}

				function CuartoOrden($x,$y,$h,$n,$funcion){
					for($i=0;$i<=$n;$i++){   
						$vectorX[$i]=$x;
						$vectorY[$i]=$y;
						$k1=funcionEval($x,$y,$funcion);
						$k2=funcionEval($x+$h/2,$y+(($h*$k1)/2),$funcion);
						$k3=funcionEval($x+$h/2,$y+(($h*$k2)/2),$funcion);
						$k4=funcionEval($x+$h,$y+$h*$k3,$funcion);
						$x+=$h;
